...
app.put('/api/trips/:id', authenticateToken, async (req, res) => {
  try {
    const { title, description, startDate, endDate, destination, status } = req.body;
...
// Start server
app.listen(PORT, () => {
  console.log(`Travel Planner server with database running on http://localhost:${PORT}`);
  console.log(`Database integration: ${process.env.DATABASE_URL ? 'Enabled' : 'Disabled'}`);
});
